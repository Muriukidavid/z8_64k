#include "lcd.h"

/* sequence for Nokia3310 LCD Initialization */
void init_lcd(void){
	config_lcd(); // call gpio mappings for simulated spi
	RES() ;        // Set RES HIGH.
	CE() ;         // Disable Chip.
	CLR_RES() ;                // Reset the LCD.
	delay_ms(2400);  // Wait 100ms.
	RES() ;                // Awake LCD from RESET state.
	wrcmd(0x21);            // Activate Chip and H=1.
	wrcmd(0xC2);            // Set LCD Voltage to about 7V.
	wrcmd(0x13);            // Adjust voltage bias.
	wrcmd(0x20);            // Horizontal addressing and H=0.
	wrcmd(0x09);            // Activate all segments.
	clearram();             // Erase all pixel on the DDRAM.
	wrcmd(0x08);            // Blank the Display.
	wrcmd(0x0C);            // 0x0C Display Normal. 0x0D for inverse
	pixelxy(0,0);           // Cursor Home.
}

/* Lookup Table for Nokia LCD */
unsigned char const FontLookup [][5] ={
    { 0x00, 0x00, 0x00, 0x00, 0x00 },  // sp
    { 0x00, 0x00, 0x2f, 0x00, 0x00 },   // !{ 0x00, 0x00, 0x2f, 0x00, 0x00 },
    { 0x00, 0x07, 0x00, 0x07, 0x00 },   // "
    { 0x14, 0x7f, 0x14, 0x7f, 0x14 },   // #
    { 0x24, 0x2a, 0x7f, 0x2a, 0x12 },   // $
    { 0xc4, 0xc8, 0x10, 0x26, 0x46 },   // %
    { 0x36, 0x49, 0x55, 0x22, 0x50 },   // &
    { 0x00, 0x05, 0x03, 0x00, 0x00 },   // '
    { 0x00, 0x1c, 0x22, 0x41, 0x00 },   // (
    { 0x00, 0x41, 0x22, 0x1c, 0x00 },   // )
    { 0x14, 0x08, 0x3E, 0x08, 0x14 },   // *
    { 0x08, 0x08, 0x3E, 0x08, 0x08 },   // +
    { 0x00, 0x00, 0x50, 0x30, 0x00 },   // ,
    { 0x10, 0x10, 0x10, 0x10, 0x10 },   // -
    { 0x00, 0x60, 0x60, 0x00, 0x00 },   // .
    { 0x20, 0x10, 0x08, 0x04, 0x02 },   // /
    { 0x3E, 0x51, 0x49, 0x45, 0x3E },   // 0
    { 0x00, 0x42, 0x7F, 0x40, 0x00 },   // 1
    { 0x42, 0x61, 0x51, 0x49, 0x46 },   // 2
    { 0x21, 0x41, 0x45, 0x4B, 0x31 },   // 3
    { 0x18, 0x14, 0x12, 0x7F, 0x10 },   // 4
    { 0x27, 0x45, 0x45, 0x45, 0x39 },   // 5
    { 0x3C, 0x4A, 0x49, 0x49, 0x30 },   // 6
    { 0x01, 0x71, 0x09, 0x05, 0x03 },   // 7
    { 0x36, 0x49, 0x49, 0x49, 0x36 },   // 8
    { 0x06, 0x49, 0x49, 0x29, 0x1E },   // 9
    { 0x00, 0x36, 0x36, 0x00, 0x00 },   // :
    { 0x00, 0x56, 0x36, 0x00, 0x00 },   // ;
    { 0x08, 0x14, 0x22, 0x41, 0x00 },   // <
    { 0x14, 0x14, 0x14, 0x14, 0x14 },   // =
    { 0x00, 0x41, 0x22, 0x14, 0x08 },   // >
    { 0x02, 0x01, 0x51, 0x09, 0x06 },   // ?
    { 0x32, 0x49, 0x59, 0x51, 0x3E },   // @
    { 0x7E, 0x11, 0x11, 0x11, 0x7E },   // A
    { 0x7F, 0x49, 0x49, 0x49, 0x36 },   // B
    { 0x3E, 0x41, 0x41, 0x41, 0x22 },   // C
    { 0x7F, 0x41, 0x41, 0x22, 0x1C },   // D
    { 0x7F, 0x49, 0x49, 0x49, 0x41 },   // E
    { 0x7F, 0x09, 0x09, 0x09, 0x01 },   // F
    { 0x3E, 0x41, 0x49, 0x49, 0x7A },   // G
    { 0x7F, 0x08, 0x08, 0x08, 0x7F },   // H
    { 0x00, 0x41, 0x7F, 0x41, 0x00 },   // I
    { 0x20, 0x40, 0x41, 0x3F, 0x01 },   // J
    { 0x7F, 0x08, 0x14, 0x22, 0x41 },   // K
    { 0x7F, 0x40, 0x40, 0x40, 0x40 },   // L
    { 0x7F, 0x02, 0x0C, 0x02, 0x7F },   // M
    { 0x7F, 0x04, 0x08, 0x10, 0x7F },   // N
    { 0x3E, 0x41, 0x41, 0x41, 0x3E },   // O
    { 0x7F, 0x09, 0x09, 0x09, 0x06 },   // P
    { 0x3E, 0x41, 0x51, 0x21, 0x5E },   // Q
    { 0x7F, 0x09, 0x19, 0x29, 0x46 },   // R
    { 0x46, 0x49, 0x49, 0x49, 0x31 },   // S
    { 0x01, 0x01, 0x7F, 0x01, 0x01 },   // T
    { 0x3F, 0x40, 0x40, 0x40, 0x3F },   // U
    { 0x1F, 0x20, 0x40, 0x20, 0x1F },   // V
    { 0x3F, 0x40, 0x38, 0x40, 0x3F },   // W
    { 0x63, 0x14, 0x08, 0x14, 0x63 },   // X
    { 0x07, 0x08, 0x70, 0x08, 0x07 },   // Y
    { 0x61, 0x51, 0x49, 0x45, 0x43 },   // Z
	{ 0x00, 0x7f, 0x41, 0x41, 0x00 },	// 5b [
	{ 0x02, 0x04, 0x08, 0x10, 0x20 },	// 5c Yen Currency Sign
	{ 0x00, 0x41, 0x41, 0x7f, 0x00 },	// 5d ]
	{ 0x04, 0x02, 0x01, 0x02, 0x04 },	// 5e ^
	{ 0x40, 0x40, 0x40, 0x40, 0x40 },	// 5f _
	{ 0x00, 0x01, 0x02, 0x04, 0x00 },	// 60 `
    { 0x20, 0x54, 0x54, 0x54, 0x78 },   // a
    { 0x7F, 0x48, 0x44, 0x44, 0x38 },   // b
    { 0x38, 0x44, 0x44, 0x44, 0x20 },   // c
    { 0x38, 0x44, 0x44, 0x48, 0x7F },   // d
    { 0x38, 0x54, 0x54, 0x54, 0x18 },   // e
    { 0x08, 0x7E, 0x09, 0x01, 0x02 },   // f
    { 0x0C, 0x52, 0x52, 0x52, 0x3E },   // g
    { 0x7F, 0x08, 0x04, 0x04, 0x78 },   // h
    { 0x00, 0x44, 0x7D, 0x40, 0x00 },   // i
    { 0x20, 0x40, 0x44, 0x3D, 0x00 },   // j
    { 0x7F, 0x10, 0x28, 0x44, 0x00 },   // k
    { 0x00, 0x41, 0x7F, 0x40, 0x00 },   // l
    { 0x7C, 0x04, 0x18, 0x04, 0x78 },   // m
    { 0x7C, 0x08, 0x04, 0x04, 0x78 },   // n
    { 0x38, 0x44, 0x44, 0x44, 0x38 },   // o
    { 0x7C, 0x14, 0x14, 0x14, 0x08 },   // p
    { 0x08, 0x14, 0x14, 0x18, 0x7C },   // q
    { 0x7C, 0x08, 0x04, 0x04, 0x08 },   // r
    { 0x48, 0x54, 0x54, 0x54, 0x20 },   // s
    { 0x04, 0x3F, 0x44, 0x40, 0x20 },   // t
    { 0x3C, 0x40, 0x40, 0x20, 0x7C },   // u
    { 0x1C, 0x20, 0x40, 0x20, 0x1C },   // v
    { 0x3C, 0x40, 0x30, 0x40, 0x3C },   // w
    { 0x44, 0x28, 0x10, 0x28, 0x44 },   // x
    { 0x0C, 0x50, 0x50, 0x50, 0x3C },   // y
    { 0x44, 0x64, 0x54, 0x4C, 0x44 },   // z
	{ 0x00, 0x08, 0x36, 0x41, 0x00 },	// 7b <
	{ 0x00, 0x00, 0x7f, 0x00, 0x00 },	// 7c |
	{ 0x00, 0x41, 0x36, 0x08, 0x00 },	// 7d >
	{ 0x10, 0x08, 0x08, 0x10, 0x08 },	// 7e Right Arrow 	->
	{ 0x78, 0x46, 0x41, 0x46, 0x78 }	// 7f Left Arrow	<-
};
/*
	{ 0x00, 0x7F, 0x41, 0x41, 0x00 },   // [
    { 0x55, 0x2A, 0x55, 0x2A, 0x55 },   // 55
    //{ 0x00, 0x41, 0x41, 0x7F, 0x00 },   // ]
	{ 0x03, 0x03, 0x00, 0x00, 0x00 },   // ] DEGREES
    //{ 0x04, 0x02, 0x01, 0x02, 0x04 },   // ^ ////
	{ 0xFF, 0xFF, 0xFF, 0xFF, 0xFF },   // ^ //// modified
    { 0x40, 0x40, 0x40, 0x40, 0x40 },   // _
    { 0x00, 0x01, 0x02, 0x04, 0x00 },   // '
	*/
/* Send single byte on Bus by bitbang) */
/* Input Arguments: x -> Byte to send     */
void send(unsigned char x){
unsigned char i;
	for(i=8;i>0;i--){
		CLR_SCK();
		if((x&0x80)){
			DAT();
		} else {
			CLR_DAT();
			}
		SCK();
		x <<= 1;
	}

}

/* Write Data Byte to Nokia LCD              */
/* Input Arguments: x-> Data Byte to be sent */
void wrdata(unsigned char x){
	DC();
	CLR_CE();
	send(x);
	CE();
}

/* Write Command Byte to Nokia LCD               */
/* Input Arguments: x -> Command Byte to be sent */
void wrcmd(unsigned char x){
	CLR_DC();
	CLR_CE();
	send(x);
	CE();
}

/* Brings ram pointer to X,Y pixel position            */
/* Input Arguments: x-> X cordinate range from 0 to 83 */
/* Input Arguments: y-> Y cordinate range from 0 to 5  */
void pixelxy(unsigned char x, unsigned char y){
	wrcmd(0x40|(y&0x07));   // Y axis ** changed from (y+1) to y to enable writing to first row
	wrcmd(0x80|(x&0x7f));   // X axis
}

/* Clears the screen */
void clearram(void){
	int ddram;
	pixelxy(0,0);                                                                           // Cursor Home.
	for (ddram=800;ddram>0;ddram--) {wrdata(0x00);}     // 6*84 = 504 DDRAM addresses.
}




/* Bring cursor to Line and character specified                     */
/* Input Arguments: row    -> Line number range from 1 to 6         */
/* Input Arguments: Column -> character position range from 1 to 14 */
/* You can have maximum of 6 lines of text on LCD and each line containing 14 characters. Address is auto increment              */
void cursorxy(unsigned char row, unsigned char col){
	if((row>6) || (row<1) || (col<1) || (col>14))
		return;
	pixelxy(((col-1)*6),(row-1));
}

//set LCD contrast
void lcd_contrast(unsigned char contrast){
	//  LCD Extended Commands.
	wrcmd(0x21);
	//Set LCD Vop (Contrast).
	wrcmd( 0x80 | contrast);
	//LCD Standard Commands, horizontal addressing mode.
	wrcmd( 0x20);
}

/* Writes single character on LCD                   */
/* Input Arguments: ch -> Character to be displayed */
void putchar_1(unsigned char ch){
	unsigned char i, temp;
	if ( (ch < 0x20) || (ch > 0x7b) ){
	ch = 92;
	}
	for(i=0;i<5;i++){
	temp = FontLookup[ch - 32][i] << 1;
	wrdata(temp);
	}
	wrdata(0x00);
}

/*writes an integer to LCD*/
void putint(int in, char digits){
	char x=digits-1;
	while(x>=0){
		putchar_1((char)(in/pow(10,x)%10)+48);
		x=x-1;		
	}
} 

//display a floating point value, 4 digits, 2 decimal places
void putfloat(float in){
	int temp1,temp2;
	temp1=(int)in%100;
	temp2=(int)(in*100);
	temp2=temp2%10000;
	temp2=temp2-(temp1*100);
	putint((int)(temp1), 2);
	putstr((unsigned char*)("."));
	putint((int)(temp2), 2);
}

/* Writes character string on LCD                            */
/* Input Arguments: str -> Pointer to string to be displayed */
void putstr (unsigned char *str){
	while(*str)
	putchar_1(*str++);
}

//positive powers of integers
int pow(int x,char y){
	char i=1;
	int xx=x;
	if(y==0){
		return 1;
	}else{
		while(i<y){
			xx=xx*xx;
			i=i+1;
		}
		return xx;
	}
}

void clear(char x, char y, char xters){
	int i=0;
	cursorxy(x,y);
	//clear float data, 5 digits = 5*5 =25 bytes
	for(i=0; i<(xters*5); i++){
		wrdata(0x00);
		}
}